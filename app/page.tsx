...
const hostingFeatures = [
  ...
  { mark: "CDN", title: "Global CDN", text: "Serve cached content through a distributed delivery layer for faster visitor access around the world.", status: "Active" },
...
const products = [
  { mark: "WEB", title: "Shared Web Hosting", outcome: "A dependable home for your website.", meta: "From $7.99/mo", text: "Autoscaling shared-cloud hosting with SSL, CDN, backups, security tools, DNS controls, and account management together.", href: "#pricing" },
...
const footerColumns = [
  { title: "Hosting", links: [["Shared Web Hosting", "#pricing"], ["Managed Cloud", "#scale"], ["VPS Hosting", "#scale"], ["Managed WordPress", "#products"], ["Global Infrastructure", "#network"]] },
  ...
  { title: "Website Services", links: [["AI Website Builder", "#ai-builder"], ["Website Migration", "#switching"], ["Done-for-You Migration", "#switching"], ["Custom Setup", "#custom"], ["Multi-Site Moves", "#custom"]] },
  ...
  return (
    <main id="top" className="hmw-page">
      <header className="site-header hmw-header">
        ...
        <nav aria-label="Primary navigation"><a href="#pricing">Hosting</a><a href="#scale">Scale</a><a href="#products">Products</a><a href="#network">Network</a><a href="#domains">Domains</a><a href="#support">Support</a></nav>
      ...
      <section className="hmw-command-hero">
        <div className="hmw-hero-copy">
          ...
          <div className="hmw-hero-actions"><a className="primary-button" href="#pricing">View Shared Hosting <span>→</span></a><a className="hmw-outline-button" href="#scale">See the Scale Path</a></div>
      ...
      <section className="hmw-telemetry-bar" aria-label="HostMyWeb hosting telemetry"><div className="hmw-telemetry-lead"><i /> SYSTEM ONLINE</div><div><small>HOSTING MODEL</small><b>AUTOSCALING</b></div><div><small>STARTER RATE</small><b>$7.99/MO</b></div><div><small>GLOBAL CDN</small><b>AVAILABLE</b></div><div><small>RENEWAL SURGE</small><b>NONE</b></div><div><small>SCALE PATH</small><b>CLOUD + VPS</b></div></section>
      ...
      <section className="hmw-network-section" id="network">
        ...
        <div className="hmw-network-console"><div className="hmw-workstation-head"><div><i /> NETWORK OPERATIONS</div><span>AUTOSCALING CLOUD</span></div><div className="hmw-network-grid"><article><small>COMPUTE</small><b>Autoscaling</b><span>Capacity adapts as site demand changes.</span><em>ACTIVE</em></article><article><small>TRAFFIC</small><b>Load balanced</b><span>Requests are distributed across the hosting platform.</span><em>ACTIVE</em></article><article><small>AVAILABILITY</small><b>Redundant services</b><span>Core hosting roles are designed for high availability.</span><em>ACTIVE</em></article><article><small>DELIVERY</small><b>Global CDN</b><span>Cached content can be delivered through a worldwide edge network.</span><em>AVAILABLE</em></article></div><div className="hmw-region-strip"><div><small>ORIGIN OPTIONS</small><b>Choose closer to your audience</b></div><span><i /> USA</span><span><i /> UNITED KINGDOM</span><span><i /> SINGAPORE</span><em>Origin availability is selected during service setup and can depend on the product configuration.</em></div></div>
      </section>

      <section className="hmw-domain-section" id="domains"><div className="hmw-section-intro hmw-section-intro-light"><div className="hmw-section-kicker"><i /> DOMAIN ACQUISITION</div><h2>Find the right domain.<br /><em>See the renewal before you buy.</em></h2><p>Search current availability, then create an account to complete registration. HostMyWeb shows the registration and renewal price together so a first-year promotion does not hide what the domain costs later.</p><div className="hmw-mini-telemetry"><span><b>01</b> Search availability</span><span><b>02</b> See registration price</span><span><b>03</b> See renewal price</span></div></div><div className="hmw-domain-workstation"><div className="hmw-workstation-head"><div><i /> DOMAIN CONTROL</div><span>LIVE SEARCH</span></div><div className="hmw-domain-search-shell"><DomainSearch /></div><div className="hmw-domain-price-grid">{domainPrices.map((item) => <article key={item.tld}><div><small>TLD</small><b>{item.tld}</b></div><strong>{item.price}<span>/yr</span></strong><p>{item.note}</p><em>PRICE VISIBLE</em></article>)}</div></div></section>

      <section className="hmw-pricing-section" id="pricing"><div className="hmw-section-heading-row"><div className="hmw-section-intro"><div className="hmw-section-kicker"><i /> AUTOSCALING SHARED CLOUD PLANS</div><h2>Pick the capacity.<br /><em>Keep the price.</em></h2><p>The product is shared web hosting, but the underlying platform is autoscaling and load balanced rather than a traditional single-server shared environment. Each plan has one real monthly hosting rate.</p></div><div className="hmw-price-lock-panel"><small>HOSTMYWEB PRICE LOCK</small><strong>The price you sign up for is the price you keep.</strong><p>The base hosting subscription stays at the same monthly rate while you continuously keep that same plan active.</p><a href="#price-lock">Review Price Lock →</a></div></div><div className="hmw-plan-grid">{plans.map((plan) => <article className={plan.popular ? "hmw-plan-card hmw-plan-featured" : "hmw-plan-card"} key={plan.name}><div className="hmw-plan-head"><span>{plan.code}</span><em>{plan.popular ? "RECOMMENDED" : "PRICE LOCKED"}</em></div><small>{plan.fit}</small><h3>{plan.name}</h3><div className="hmw-plan-price"><b>{plan.price}</b><span>{plan.unit}</span></div><div className="hmw-plan-renewal"><span>Today</span><b>{plan.price}</b><i /> <span>Renewal</span><b>{plan.price}</b></div><p>{plan.description}</p><ul>{plan.features.map((item) => <li key={item}><i />{item}</li>)}</ul><div className="hmw-plan-mail-note">Need more email? Add business mailboxes separately without changing your hosting plan.</div><a href={plan.href}>Choose {plan.name} <span>→</span></a></article>)}</div></section>
      ...
      <section className="hmw-custom-section" id="custom"><div className="hmw-custom-copy"><div className="hmw-section-kicker"><i /> CUSTOM OPERATIONS REQUEST</div><h2>Need managed cloud, a VPS, multi-site migration, or an unusual setup?</h2><p>Use this request channel for configurations that need capacity planning, server choices, migration work, or human review.</p><div className="hmw-custom-checks"><span>✓ Managed cloud</span><span>✓ Virtual private servers</span><span>✓ Multi-site moves</span><span>✓ Custom infrastructure</span></div></div><div className="hmw-contact-shell"><div className="hmw-workstation-head"><div><i /> REQUEST INTAKE</div><span>HUMAN REVIEW</span></div><ContactForm /></div></section>

      <footer className="hmw-footer"><div className="hmw-footer-command"><div><div className="hmw-section-kicker hmw-kicker-green"><i /> HOSTMYWEB PRICE LOCK</div><h2>Start small. Scale without leaving.</h2><p>Transparent shared hosting today, with managed cloud and VPS paths available when the project needs a different kind of infrastructure.</p></div><div className="hmw-footer-actions"><a className="primary-button" href="#pricing">View Shared Hosting →</a><a className="hmw-outline-button hmw-outline-light" href="#scale">See Scale Options</a></div></div><div className="hmw-footer-status"><div className="hmw-footer-brand"><a className="brand" href="#top"><span className="brand-mark"><i /><i /><i /></span><span><b>HostMyWeb</b><small>HOSTING OPERATIONS</small></span></a><p>Autoscaling shared-cloud hosting, domains, email, website services, and a clear path into managed cloud and VPS infrastructure.</p></div><div className="hmw-footer-telemetry"><article><small>PRICE LOCK</small><b>ACTIVE</b><span>Base shared-hosting rate stays the same</span></article><article><small>GLOBAL DELIVERY</small><b>AVAILABLE</b><span>CDN plus selectable origin regions</span></article><article><small>SCALE PATH</small><b>READY</b><span>Shared cloud → managed cloud → VPS</span></article></div></div><div className="hmw-footer-links">{footerColumns.map((column) => <div key={column.title}><b>{column.title}</b>{column.links.map(([label, href]) => <a href={href} key={label} target={href.startsWith("http") ? "_blank" : undefined} rel={href.startsWith("http") ? "noreferrer" : undefined}>{label}</a>)}</div>)}</div><div className="hmw-footer-bottom"><span>© {new Date().getFullYear()} HostMyWeb.co. All rights reserved.</span><div><b><i /> SYSTEM ONLINE</b><span>Price Lock active</span><span>Autoscaling hosting</span><span>Cloud + VPS scale path</span></div></div></footer>
    </main>
  );
}
